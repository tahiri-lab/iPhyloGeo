...
def prepareDirectory():
    ...
    path_output_windows = 'user/' + getIpAdress() +'/output/seq/windows'                            
    ...
    delete_path = os.listdir('user/' + getIpAdress() +'/output/seq')

    for item in delete_path:
        if item.endswith("_gene"):
            shutil.rmtree('user/' + getIpAdress() +'/output/seq'+'/'+item)
        

    delete_path2 = 'user/' + getIpAdress() + '/output/'
...
def createPhylogeneticTree(reference_gene_file, window_size, step_size, bootstrap_threshold, rf_threshold, data_names):
    prepareDirectory()
    os.chdir('user/' + getIpAdress() + '/output/')       #Requires access to the specific 'output' folder
